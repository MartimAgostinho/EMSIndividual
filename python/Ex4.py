...
cutoff_idx       = np.where(magnitude_db <= 20*np.log10( hz[0] )-3)[0][0]
...
plt.axvline(x=cutoff_frequency, color='r', linestyle='--', linewidth=1)
...
# Bode Phase response
plt.subplot(2, 1, 2)
plt.semilogx(wz/(2*np.pi), np.angle(hz,deg=True))
